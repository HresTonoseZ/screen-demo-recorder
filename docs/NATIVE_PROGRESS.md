...
- Native GIF export from the completed MP4, including the same burned-in label
  ...
  No Python process is used by the native pipeline. Native builds validate and
  package the pinned LGPL FFmpeg runtime stored in this repository for CPU video processing.
